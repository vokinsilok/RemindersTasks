...
async def process_due_reminders(client: VkClient) -> None:
    now = datetime.now()
    async with async_session_maker() as session:
        repo = ReminderRepository(session)
        due_reminders = await repo.due(now, platform="vk")

        for reminder in due_reminders:
            if reminder.user.vk_user_id is None:
                continue

            try:
                await client.send_message(
                    peer_id=reminder.user.vk_user_id,
                    message=build_reminder_message(reminder),
                )
            except VkApiError as exc:
                logger.warning("Failed to send VK reminder %s: %s", reminder.id, exc)
                continue
...
async def run_scheduler(client: VkClient) -> None:
    settings = get_settings()
    while True:
        try:
            await process_due_reminders(client)
        except Exception:
            logger.exception("VK scheduler iteration failed")
